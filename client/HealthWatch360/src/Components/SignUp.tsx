...
export const SignUp = () => {
    const schema = yup.object().shape({
        ...
        role: yup.string().required("Enter the Role").matches(/(admin|doctor|patient)/, "Enter a valid role"),
    ...
    const onSubmit = async (data: any) => {
        try {
            const response = await apiClient.post("/auth/signup", { email: data.email, password: data.password, nic: data.nic, name: data.name, role: data.role, bod: data.bod});
                ...
                <main className="w-72 text-center">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="d-form-control mb-2">
                            ...
                            <input id="email-input" type="text" placeholder="nic" className="d-input d-input-bordered" {...register("nic")} />
                            {errors.nic && <p className="text-red-500" style={{ color: "red", fontSize: "12px" }}>{errors.nic.message}</p>}
                        </div>
                        <div className="d-form-control mb-2">
                            <label className="d-label" htmlFor="email-input">
                                <span className="d-label-text">Name</span>
                            </label>
                            <input id="email-input" type="text" placeholder="Enter the Name" className="d-input d-input-bordered" {...register("name")} />
                            {errors.name && <p className="text-red-500" style={{ color: "red", fontSize: "12px" }}>{errors.name.message}</p>}
                        </div>
                        <div className="d-form-control mb-2">
                            <label className="d-label" htmlFor="email-input">
                                <span className="d-label-text">Role</span>
                            </label>
                            <select id="role-input" className="d-input d-input-bordered" {...register("role")}>
                                <option value="">Select Role</option>
                                <option value="admin">Admin</option>
                                <option value="doctor">Doctor</option>
                                <option value="patient">Patient</option>
                            </select>
                            {errors.role && <p className="text-red-500" style={{ color: "red", fontSize: "12px" }}>{errors.role.message}</p>}
                        </div>
                        <div className="d-form-control mb-2">
                            <label className="d-label" htmlFor="email-input">
                                <span className="d-label-text">Date of Birth</span>
                            </label>
                            <input id="email-input" type="date" placeholder="bod" className="d-input d-input-bordered" {...register("bod")} />
                            {errors.bod && <p className="text-red-500" style={{ color: "red", fontSize: "12px" }}>{errors.bod.message}</p>}
                        </div>
                        <div className="d-form-control mb-2">
                            <label className="d-label" htmlFor="email-input">
